  ...
  return (
    <div style={{ padding: "32px 36px", minHeight: "100vh" }}>
      ...
      <div className="no-print" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "#111827", margin: 0, letterSpacing: -0.5 }}>QR Codes</h1>
          <p style={{ color: "#6B7280", marginTop: 4, fontSize: 14 }}>Download student QR codes for ID cards & attendance</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={printAll} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", border: "1px solid #E5E7EB", borderRadius: 10, background: "#fff", color: "#374151", fontSize: 13, fontWeight: 600, cursor: "pointer", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
            Print All
          </button>
          <button onClick={downloadAll} disabled={filtered.length === 0} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", border: "none", borderRadius: 10, background: "linear-gradient(135deg,#4F46E5,#7C3AED)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: filtered.length === 0 ? "not-allowed" : "pointer", boxShadow: "0 4px 12px rgba(79,70,229,0.35)", opacity: filtered.length === 0 ? 0.6 : 1 }}>
      ...
      <div className="no-print" style={{ display: "flex", gap: 12, marginBottom: 24, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, maxWidth: 360 }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "9px 12px 9px 36px", border: "1px solid #E5E7EB", borderRadius: 10, fontSize: 14, boxSizing: "border-box", outline: "none", color: "#111827", background: "#fff" }}
          />
        </div>
        ...
        <div style={{ marginLeft: "auto", fontSize: 13, color: "#9CA3AF" }}>{filtered.length} student{filtered.length !== 1 ? "s" : ""}</div>
      </div>
